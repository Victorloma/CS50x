...
def index():
    ...
    # Loop over portfolio to calculate balance
    for stock in stocks:
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("must provide a positive round number of shares")
        ...
        flash(f"Bought {shares} share{'s' if int(shares) > 1 else ''} of {symbol} for {usd(total_cost)}!")
        return redirect("/")
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure password was submitted
            ...
            return apology("must provide password and must confirm password", 400)

        elif request.form.get("password") != request.form.get("confirmation"):
            return apology("password must be the same as confirmation password")
...
def sell():
    ...
    if request.method == "POST":
        ...
        shares = request.form.get("shares")
        if not symbol:
            return apology("pleace provide a stock ticker")
        elif not shares or not shares.isdigit() or int(shares) <= 0:
            return apology("you must at least sell 1 share")
        else:
            shares = int(shares)
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    flash(f"Sold {shares} share{'s' if shares > 1 else ''} of {symbol} for {usd(total_sale)}!")
                    return redirect("/")
